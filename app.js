/* سوبر ماركت أولاد يحيى — دفتر الحسابات
   ...
   ✅ حماية من اختفاء أزرار/عناصر HTML (لا Errors)
*/
...
// ✅ سجل المحذوفات داخل نفس الـ LocalStorage
const TRASH_KEY = "oy_trash_v1"; // إضافي (لو حبيت تفصله)
const MAX_TRASH = 1000;          // حد أقصى للوج عشان ما يكبرش زيادة
...
/*
  ملاحظة:
  - أنت هتشيل أزرار الرجوع، فالتنقل هيكون من التابات.
  - لو أضفت صفحة trash: حط section id="page-trash"
*/
const PAGES = [
...
function openEntryPreview(entryId){
  ...
  el("prevEntryMeta") && (el("prevEntryMeta").textContent = `تاريخ الإنشاء: ${new Date(entry.createdAt).toLocaleString("ar-EG")}`);
  el("prevEntryStatus") && (el("prevEntryStatus").textContent = (v.remaining === 0) ? "الحالة: مقفول" : `الحالة: عليه باقي ${fmt(v.remaining)} ج.م`);
...
function showLedger(name){
  ...
  // ✅ بحث جزئي: contains بدل التطابق الكامل
  ...
  // ✅ تفاصيل العمليات: كل عملية سطر واضح
...
function renderTrash(){
  ...
  if(logs.length === 0){
    tbody.innerHTML = `<tr><td colspan="6">لا توجد محذوفات.</td></tr>`;
    return;
  }
...
/* -------------------- Export/Import (لو شلتهم من HTML مش هيعمل Error) -------------------- */
...
function refresh(){
  ...
  // لو المستخدم واقف في صفحة trash
...
document.addEventListener("DOMContentLoaded", () => {
  try{
    ...
    // ✅ Auto open after refresh (same tab)
    if(isAuthed()){
      openApp();
    }
    ...
    // ✅ Logout (لو شلت غيره من الهيدر مش مشكلة)
    ...
    // ✅ firstPay: لو المستخدم كتب 0 يمسحه
    ...
    el("entryForm")?.addEventListener("submit", (ev)=>{
      ...
      // ✅ لازم نتحقق من الدفعة قبل حفظ العملية
      if(firstPay !== null){
    ...
    // ✅ Clear All (تم إيقافه حتى لو الزر موجود)
    el("btnClearAll")?.addEventListener("click", ()=>{
      alert("تم تعطيل زر حذف كل البيانات حفاظًا على السجل.");
    });

    // ✅ Export/Import (لو موجودين فقط)
    ...
    // ✅ Ledger: بحث مباشر أثناء الكتابة + بحث جزئي
